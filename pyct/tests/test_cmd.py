...
# Same as in pyct/examples/data/.data_stubs/foo.csv
...
def tmp_project_with_examples(tmp_path):
    ...
    (examples / "data").mkdir()
    example = examples / "Foo_Notebook.ipynb"
...
def tmp_project_with_test_file(tmp_project_with_stubs):
    ...
    data_stub = project /  "examples" / "data" / ".data_stubs" / "foo.csv"
...
def test_examples_with_use_test_data(tmp_project):
    ...
    assert (project / "examples" / "data" / "foo.csv").is_file()
    assert (project / "examples" / "Foo_Notebook.ipynb").is_file()

def test_examples_with_prexisting_content_in_target_raises_error(tmp_project_with_examples):
    project = tmp_project_with_examples
    path = str(project / "examples")
    data = project / "examples" / "data" / "foo.csv"
    data.write_text(FOO_REGULAR_FILE_CONTENT)
    with pytest.raises(ValueError):
        examples(name="pyct", path=path, use_test_data=True)
    assert (project / "examples" / "Foo_Notebook.ipynb").is_file()
    assert (project / "examples" / "Foo_Notebook.ipynb").read_text() == FAKE_EXAMPLE_CONTENT
    assert (project / "examples" / "data" / "foo.csv").is_file()
    assert (project / "examples" / "data" / "foo.csv").read_text() == FOO_REGULAR_FILE_CONTENT
...
def test_fetch_data_using_test_data_with_no_file_in_data_copies_from_stubs(tmp_project_with_test_file):
    ...
    assert (project / "examples" / "data" / "foo.csv").is_file()
    assert (project / "examples" / "data" / "foo.csv").read_text() == FOO_TEST_FILE_CONTENT
...
def test_clean_data_when_data_file_is_from_stubs_removes_file_from_data(tmp_project_with_test_file):
    ...
    clean_data(name=name, path=path)
    assert not (project / "examples" / "data" / "foo.csv").is_file()
    assert (project / "examples" / "data" / ".data_stubs" / "foo.csv").is_file()
    assert (project / "examples" / "data" / ".data_stubs" / "foo.csv").read_text() == FOO_TEST_FILE_CONTENT
...
def test_clean_data_when_stubs_is_empty_does_nothing(tmp_project_with_stubs):
    ...
    clean_data(name=name, path=path)
    assert (project / "examples" / "data" / "foo.csv").is_file()
    assert not (project / "examples" / "data" / ".data_stubs" / "foo.csv").is_file()
